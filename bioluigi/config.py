...
class bioluigi(luigi.Config):
    scheduler = luigi.Parameter(default='local', significant=False,
                                description='Default scheduler to use in ScheduledExternalProgram')
    scheduler_partition = luigi.OptionalParameter(default='', significant=False,
                                                  description='Node partition to use for scheduling jobs if supported')
    scheduler_extra_args = luigi.ListParameter(default=[], significant=False,
